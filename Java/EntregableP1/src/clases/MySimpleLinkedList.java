...
public class MySimpleLinkedList {
	...
	public MySimpleLinkedList getSecuences() {
		//SOLO FUNCIOMA CON NUMEROS ENTEROS. Retorna una lista de listas donde cada nodo es una subsecuencia de numeros ordenados ascendentemente.
		MySimpleLinkedList tmp = new MySimpleLinkedList();
		MySimpleLinkedList solution = new MySimpleLinkedList();
		Node aux = this.first;
		while(aux != null) {
			if ((tmp.isEmpty()) || ((int)tmp.last.getInfo() < (int)aux.getInfo())) {
				tmp.insertBack(aux.getInfo());
				aux = aux.getNext();
			}
			else {
				if(tmp.size() >= 2) {
					solution.insertBack(tmp);
				}
